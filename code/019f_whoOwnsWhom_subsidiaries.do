...
------------------------------+-----------------------------------
          only in master data |      1,508        0.15        0.15 -- 307 gvkeys that appear in the data of Arora, Belenzon, and Sheer (2021) or the WRDS-SEC 10-K filing data, but not in our Compustat data. Possibly 2021 listings.
           only in using data |     33,168        3.30        3.45 -- 31,864 gvkeys that do not feature in any of our subsidiary ownership datasets
both in master and using data |    970,978       96.55      100.00 -- 9,938 gvkeys that feature both in our version of Compustat and at least one of our subsidiary datasets
------------------------------+-----------------------------------
                        Total |  1,005,654      100.00
*/
...
* Take Narrow Boundaries for Subsidiary -to- gvkeyUO Mappings *

gen year1_uo = max(year1, year1_subs)

label var year1_uo "First year in which subsidiary is ultimately owned by gvkeyUO"

gen yearN_uo = min(yearN, yearN_subs)

label var yearN_uo "Last year in which subsidiary is ultimately owned by gvkeyUO"
...
bysort clean_name gvkeyUO subsidiary: gen year = year1_uo + _n - 1

label var year "Year in which subsidiary is owned by gvkeyUO"
...
quietly unique gvkeyUO, by(clean_name year) gen(nrOwners)
...
label var nrOwners "Number of gvkeyUOs implied to own subsidiary in given year"
...
* Remove All Subsidiary Names Implicitly Owned by Multiple gvkeyUOs in a Given Year *
...
/*
    ...
    -----------------------------------------
    Not matched                     7,211,256
        from master                   536,651  (_merge==1) -- The 536,651 subsidiary clean names that don't merge to the patent data
        from using                  6,674,605  (_merge==2) -- The 612,173 clean names that feature in the patent data that aren't subsidiary names

    Matched                         2,181,223  (_merge==3) -- The 21,357 subsidiary clean names that *do* feature in our patent data (note we're yet to remove subsidiaries that share clean_names with their owners)
    -----------------------------------------
*/
...
/*
At this juncture, we can use the below code to check for M&A activity using subsidiary data...

bysort clean_name: egen isTradingName = max(isTradingNameObs)
keep if isTradingNameObs == 0 & isTradingName == 1
drop isTradingName isTradingNameObs
drop if length(clean_name) < 5
rename clean_name word
merge 1:1 word using "data/019e_oneHundredThousandWords.dta", keep(1)
drop _merge
...
drop if _merge == 1 
drop _merge
egen y1 = rowmax(year1_immediate year1)
egen yN = rowmin(yearN_immediate yearN)
drop year1 yearN gvk*_laterDivorced
replace gvkey_primary = "" if gvkey_primary == gvkey_secondary
rename gvkey_secondary gvkey0
rename gvkey_primary gvkey1
local loopCounter = 1
foreach V of varlist gvkeyIntTier*{
	local loopCounter = `loopCounter' + 1
	rename `V' gvkey`loopCounter'
}
reshape long gvkey, i(clean_name y1 yN year1_immediate yearN_immediate)
drop if missing(gvkey)
drop _j
rename clean_name clean_nameSubs
joinby gvkey using "$data/019e_dynamicNamesClean_matched.dta", unmatched(master)
drop _merge
drop if y1 > yofd(wb_dateN) | yN  < yofd(wb_date1)
bysort clean_name: egen to_drop = max(clean_nameSubs == clean_name)
drop if to_drop
keep clean_nameSubs year1_immediate yearN_immediate
rename clean_nameSubs clean_name
duplicates drop
merge 1:1 clean_name year1_immediate yearN_immediate using "$data/temp2.dta", keep(3)
drop if name_source != "SEC-WRDS 10-K"
drop name_source _merge yearN_immediate
joinby clean_name using "$data/019e_dynamicNamesClean_matched.dta", unmatched(using)
bysort gvkey: egen MM = max(_merge)
drop if MM < 3
drop MM
keep if yofd(wb_dateN) == year1_immediate | yofd(wb_dateN) == year1_immediate - 1
drop if gvkey_secondary == gvkey
rename gvkey_secondary gvkey_primary
rename gvkey gvkey_secondary
rename year1_immediate year
keep gvkey_primary gvkey_secondary year
duplicates drop
merge 1:1 gvkey_primary gvkey_secondary year using "$data/019d_listedListed_EA.dta", keep(1) keepusing(gvkey_primary gvkey_secondary year)
drop _merge
forvalues i = 1/`=_N'{
	di `"// `=year[`i']'"'
	di `"// Buyer"'
	di `"list if gvkey == "`=gvkey_primary[`i']'""'
	di `"// Bought"'
	di `"list if gvkey == "`=gvkey_secondary[`i']'""'
	di " "
	di "----- `i' of `=_N' complete -----"
	di ""
}
...
gen source = "Subsidiary Data - Manual"
gen type = "Merger or Acquisition"
drop if gvkey_primary == "001478" & gvkey_secondary == "001430"
drop if gvkey_primary == "004242" & gvkey_secondary == "141138"
drop if gvkey_primary == "005464" & gvkey_secondary == "012204"
drop if gvkey_primary == "005878" & gvkey_secondary == "002457"
drop if gvkey_primary == "006135" & gvkey_secondary == "014392"
drop if gvkey_primary == "007536" & gvkey_secondary == "014881"
drop if gvkey_primary == "012124" & gvkey_secondary == "001081"
drop if gvkey_primary == "013714" & gvkey_secondary == "011436"
drop if gvkey_primary == "015824" & gvkey_secondary == "110329"
drop if gvkey_primary == "028940" & gvkey_secondary == "029287"
drop if gvkey_primary == "063769" & gvkey_secondary == "008960"
drop if gvkey_primary == "065683" & gvkey_secondary == "063158"
drop if gvkey_primary == "179817" & gvkey_secondary == "178266"
drop if gvkey_primary == "006135" & gvkey_secondary == "176282"
drop if gvkey_primary == "004503" & gvkey_secondary == "007475"
drop if gvkey_primary == "018484" & gvkey_secondary == "138004"
drop if gvkey_primary == "063866" & gvkey_secondary == "184114"
replace type = "Subsidiary Listing" if gvkey_primary == "008093" & gvkey_secondary == "015760"
replace year = 1985 if gvkey_primary == "008093" & gvkey_secondary == "015760"
replace type = "Subsidiary Listing" if gvkey_primary == "011306" & gvkey_secondary == "012694"
replace year = 1985 if gvkey_primary == "011306" & gvkey_secondary == "012694"
replace year = 1989 if gvkey_primary == "004060" & gvkey_secondary == "007040"
replace year = 1992 if gvkey_primary == "007647" & gvkey_secondary == "017193"
order gvkey_primary gvkey_secondary year source type
export excel "$data/temp.xlsx", replace
*/
...
/*
	...
	Matched                            29,156  (_merge==3) -- The 16,410 (2.77% of) clean names that we retain
	-----------------------------------------
*/
keep if _merge == 3
...
/*
    ...
    -----------------------------------------
    Not matched                           103
        from master                       103  (_merge==1) -- For 103 patenters, the USPTO does not provide dates for any of their patents, so their patents do not appear in the "Homgenised Patents with Dates" dataset.
        from using               NOT RETAINED  (_merge==2)

    Matched                           423,377  (_merge==3) -- 11,672 patenters for whom at least one patent is given a date by the USPTO.
    -----------------------------------------
*/
drop if _merge == 1 // We can't use the patents of these firms anyway; see above.
...
/*
	...
	-----------------------------------------
	Not matched                       849,152
		from master                         0  (_merge==1) -- All from the master merge by construction.
		from using                    849,152  (_merge==2) -- All the subsidiaries with clean names that have already been excluded from contention.

	Matched                            18,321  (_merge==3) -- All the subsidiaries with clean names that we're still considering using in our final data at this point in the code.
	-----------------------------------------
*/
keep if _merge == 3 // See above
...
save "$data/019f_subsidiariesCut.dta", replace
...
use "$data/019f_subsidiariesCut.dta", clear


* Pick a Random Name to Use an Example from the Subsidiary Data *

// For example, the clean name HANSGROHE has 32 different unclean names of subsidiaries mapping to it. We can't retain all of these
...
drop _merge gvkeyIntT* gvkS_laterDivorced gvkT*

rename gvkey_secondary gvkey

rename gvkey_primary gvkeyUO

label var year1 "First year of continuous ownership of gvkey by gvkeyUO"

label var yearN "Last year of continuous ownership of gvkey by gvkeyUO"
...
* Get First and Last Years Subsidiary is Owned by gvkeyUO *

gen year1_uo = max(year1, year1_immediate)

label var year1_uo "First year of continuous ownership of clean_name by gvkeyUO via gvkey"

gen yearN_uo = min(yearN, yearN_immediate)

label var yearN_uo "Last year of continuous ownership of clean_name by gvkeyUO via gvkey"


* Drop Clean Names that Undergo Simultaneous Change of gvkey, gvkeyUO, and Original Subsidiary Name *

bysort clean_name (year1_uo): gen fullChangeFlag = (_n > 1 & gvkey != gvkey[_n-1] & gvkeyUO != gvkeyUO[_n-1] & upper(subs_name) != upper(subs_name[_n-1])) // Flags if previous or next change is "full ownership change"

bysort clean_name (fullChangeFlag): replace fullChangeFlag = fullChangeFlag[_N] // Makes flag uniform at clean_name level

label var fullChangeFlag "clean_name simultaneously changes both gvkey and gvkeyUO at some point"
...
save "$data/019f_subsidiariesCleanedAndCutAgain.dta", replace
...
use "$data/019f_subsidiariesCleanedAndCutAgain.dta", clear
...
* Get First and Last Years Subsidiary Maps to gvkeyUO via gvkey *
...
label var gvkeyUO "Ultimate owner of gvkey_secondary"
...
* Get First and Last Year of Link Between Clean Name and *gvkeyUO* *

bysort clean_name (year1): gen uo_ownershipRun = 1 if _n == 1

bysort clean_name (year1): replace uo_ownershipRun = uo_ownershipRun[_n-1] + (gvkeyUO != gvkeyUO[_n-1]) if _n > 1 
...
label var cnLink_y1 "First year of link between gvkeyUO and clean_name"

bysort clean_name uo_ownershipRun (yearN): egen cnLink_yN = max(yearN)

label var cnLink_yN "Last year of link between gvkeyUO and clean_name"

drop uo_ownershipRun // No longer needed


* Flag if Clean Name Maps to gvkeyUO through a Single gvkey *

bysort clean_name gvkeyUO cnLink_y1 cnLink_yN: egen singlePublicSubs = min(_n == 1 | gvkey_secondary == gvkey_secondary[_n-1]) // Equals one if there's only a single gvkey in the clean_name-gvkeyUO-cnLink_y1-cnLink_yN grouping

label var singlePublicSubs "clean_name maps to gvkeyUO through single subsidiary gvkey (possibly self)" // I think this needs re-writing at some point; see "XIOTECH"
...
label var gvkeyCNoriginator "Sole public subsidiary (possibly self) mapping clean_name to gvkeyUO"
...
label var cn_nrMappings "Number of distinct mappings from clean_name to *any* gvkeyUO"
...
gen ps_laterDivorced = 0 if singlePublicSubs == 1 // We only consider this relevant where the clean name is mapped to gvkeyUO via a single public owner.
		
local replaceCondStr = "" // We use this to populate ps_laterDivorced

quietly ds, has(varl "*Intermediary gvkey*") // Gets all gvkeyInterTier* variables into `r(varlist)'

foreach V in `=r(varlist)'{ // Loop over all all gvkeyInterTier* variables
	
	if("`replaceCondStr'" != ""){
		
		local replaceCondStr = "`replaceCondStr' &"
		
	}
	
	local replaceCondStr = "`replaceCondStr' gvkey_secondary != comparison_`V'"
...
forvalues i = 1/`maxMappingsLessOne'{ // !!!!!!!!
	...
	forvalues j = `iPlusOne'/`maxMappings'{
		...
		quietly ds, has(varl "*Intermediary gvkey*") // Gets all gvkeyInterTier* variables into `r(varlist)'. We have to do this repeatedly.
		
		foreach V in `=r(varlist)'{ // Loop over all all gvkeyInterTier* variables 
			
			bysort clean_name (cnLink_y1): gen comparison_`V' = `V'[`j']
			
		}
		
		bysort clean_name (cnLink_y1): replace ps_laterDivorced = 1 if (_n == `i' & `j' <= _N & gvkey_secondary != gvkey_secondary[`j'] & `replaceCondStr' & gvkey_secondary != gvkeyUO[`j'] & singlePublicSubs == 1)
		
		drop comparison_* // No longer needed
...
local gfriStr = "" // We use this to initiate the gvkeyFR variable

quietly ds, has(varl "*later divorced*") // Gets all *_laterDivorced variables into `r(varlist)'

foreach V in `=r(varlist)'{ // Loop over all all *_laterDivorced variables
	
	if("`gfriStr'" != ""){
		
		local gfriStr = "`gfriStr' &"
		
	}
	
	local gfriStr = "`gfriStr' (`V' == 0 | missing(`V'))"
	
}

gen gvkeyFR = gvkeyUO if singlePublicSubs == 0 | (`gfriStr')
...
foreach V of varlist gvkeyIntTier*{ // Loop over gvkeyIntTier* variables
	...
	di "replace gvkeyFR = `V' if gvkT`loopCounter'_laterDivorced == 1 & singlePublicSubs == 1"
		
	replace gvkeyFR = `V' if gvkT`loopCounter'_laterDivorced == 1 & singlePublicSubs == 1
...
drop gvkS_laterDivorced gvkT?_laterDivorced
...
label var privateSubsidiary "clean_name maps to gvkeyUO from a private subsidiary"
...
order clean_name privateSubsidiary gvkeyUO gvkeyFR cnLink_y1 cnLink_yN singlePublicSubs gvkeyIntTier* gvkeyCNoriginator gvkey_secondary subs_name year1 yearN name_source
...
* Move Intermediary Information to Missing Where clean_name Maps to gvkeyUO Through Multiple gvkeys *
...
foreach V of varlist gvkeyIntTier*{
	...
	label var `V' "gvkey `loopCounter' of gvkeyCNoriginator, ultimately owned by gvkeyUO"
	
}


* Flag Cases where We Have Multiple gvkeyFRs within clean_name-gvkeyUO-cnLink_y1-cnLink_yN * // This occurs where (1) firm A is acquired by Firm B, then (2) Firm B lists a renewed version of firm A - firm A1, then (3) firm A1 is spun-off from firm B.

quietly unique gvkeyFR, by(clean_name gvkeyUO cnLink_y1 cnLink_yN) gen(nr_gvkeyFRs)

bysort clean_name gvkeyUO cnLink_y1 cnLink_yN (nr_gvkeyFRs): replace nr_gvkeyFRs = nr_gvkeyFRs[1] // Populates all observations within clean_name-gvkeyUO-cnLink_y1-cnLink_yN

label var nr_gvkeyFRs "Number of unique gvkeyFRs within clean_name-gvkeyUO-cnLink_y1-cnLink_yN"


* Remove Extraneous Observations within clean_name-gvkeyUO-cnLink_y1-cnLink_yN *

gen gvkeyFR_lowest = "" // Only intended to be populated for clean_name-gvkeyUO-cnLink_y1-cnLink_yN groupings with multiple gvkeyFRs

foreach V of varlist gvkeyIntTier* gvkey{ // Note that this cycles *down* through chains of ownership from the top
	
	bysort clean_name gvkeyUO cnLink_y1 cnLink_yN: egen FRas`V' = max(gvkeyFR == `V') if nr_gvkeyFRs > 1 // Generates indicator for whether "V" appears as a gvkeyFR within clean_name-gvkeyUO-cnLink_y1-cnLink_yN
	
	bysort clean_name gvkeyUO cnLink_y1 cnLink_yN: replace gvkeyFR_lowest = "" if FRas`V' == 1 // Clears gvkeyFR_lowest when a lower-level gvkeyFR found
	
	bysort clean_name gvkeyUO cnLink_y1 cnLink_yN: replace gvkeyFR_lowest = gvkeyFR if gvkeyFR == `V' & FRas`V' == 1 // Gets the lowest-level gvkeyFR into clean_name-gvkeyUO-cnLink_y1-cnLink_yN for the observation where this *is* the gvkeyFR
	
	bysort clean_name gvkeyUO cnLink_y1 cnLink_yN (gvkeyFR_lowest): replace gvkeyFR_lowest = gvkeyFR_lowest[_N] if FRas`V' == 1 // Populates other values within clean_name-gvkeyUO-cnLink_y1-cnLink_yN
	
	drop FRas`V' // Only used for reference to update gvkeyFR_lowest, no longer needed
	
}

label var gvkeyFR_lowest "Lowest gvkeyFR where multiple within clean_name-gvkeyUO-cnLink_y1-cnLink_yN"

drop if nr_gvkeyFRs > 1 & gvkeyFR != gvkeyFR_lowest // Removes the extraneous observations

drop nr_gvkeyFRs gvkeyFR_lowest
...
replace gvkeyFR = "026061B" if gvkeyUO == "026061" & cnLink_y1 < 2020 & cnLink_y1 > 1998 & clean_name != "HOTTIES" // IAC Interactive/Match Group.

replace gvkeyFR = "007536A" if gvkeyUO == "007536" & clean_name == "MONSANTOEUROPE" // Monsanto/Pharmacia
replace cnLink_yN = 2004 if gvkeyUO == "007536" & clean_name == "MONSANTOEUROPE"
drop if gvkeyUO == "008530" & (clean_name == "MONSANTOEUROPE")


* Order, Compress, Export *

order clean_name privateSubsidiary gvkeyUO gvkeyFR cnLink_y1 cnLink_yN

compress

save "$data/019f_whoOwnsWhomAndWhen_privateSubs.dta", replace

/*
To track M&A activity via the transferral of subsidiaries, we use...

use "$data/019f_whoOwnsWhomAndWhen_privateSubs.dta", clear
keep if strpos(gvkeyFR, "S") == 1
bysort gvkeyFR (gvkeyCNoriginator): gen to_keep = (_N > 1 & gvkeyCNoriginator[1] != gvkeyCNoriginator[_N])
keep if to_keep
list clean_name name1 gvkeyUO gvkeyCNoriginator cnLink_y1 cnLink_yN name1_source
*/